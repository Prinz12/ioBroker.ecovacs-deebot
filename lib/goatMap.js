...
/**
 * Flatten app-compatible track groups to SVG polylines.
 * @param {Array<object>} groups
 * @returns {Array<string>}
 */
function trackSvgPoints(groups) {
    const result = [];
        ...
        for (const lid of group.lids || []) {
            for (const trace of lid.traces || []) {
                ...
                for (let index = 0; index + 1 < trace.length; index += 2) {
                    if (Number.isFinite(trace[index]) && Number.isFinite(trace[index + 1])) {
                        points.push(`${trace[index]},${trace[index + 1]}`);
                    }
                }
                if (points.length >= 2) result.push(points.join(' '));
            }
        }
    }
    return result;
...
function renderSvg(mapData) {
    ...
    for (const points of trackSvgPoints(remaining)) {
        elements.push(`<polyline points="${points}" fill="none" stroke="#607d8b" stroke-opacity="0.7" stroke-width="110" stroke-dasharray="260 180" stroke-linecap="round" stroke-linejoin="round"/>`);
    }
    for (const points of trackSvgPoints(completed)) {
        elements.push(`<polyline points="${points}" fill="none" stroke="#2e7d32" stroke-opacity="0.68" stroke-width="360" stroke-linecap="round" stroke-linejoin="round"/>`);
    }
    const overlayStyles = [
...
/**
 * Merge completed track updates without losing earlier segments from the same mow.
 * @param {Array<object>} current
 * @param {Array<object>} incoming
 * @returns {Array<object>}
 */
function mergeCompletedTracks(current, incoming) {
    const groups = new Map((current || []).map(group => [
        `${group.zid}_${group.pathType}`,
        { ...group, lids: [...(group.lids || [])] }
    ]));
    for (const group of incoming || []) {
        const key = `${group.zid}_${group.pathType}`;
        if (!groups.has(key)) groups.set(key, { ...group, lids: [] });
        const target = groups.get(key);
        for (const lid of group.lids || []) {
            const index = target.lids.findIndex(item => item.lid === lid.lid);
            if (index < 0) {
                target.lids.push(lid);
                continue;
            }
            const existing = target.lids[index];
            const traces = [...(existing.traces || [])];
            const known = new Set(traces.map(trace => JSON.stringify(trace)));
            for (const trace of lid.traces || []) {
            ...
            target.lids[index] = { ...existing, traces };
        }
    }
    ...
    if (trackSource && typeof trackSource.info === 'string') {
        ...
        try {
            ...
            for (const category of ['nonScheduledCompleted', 'scheduledCompleted']) {
                mapData.tracks[category] = mergeCompletedTracks(
...
function writeMapStates(ctx, includeStaticMap = true) {
    ...
    const svg = renderSvg(mapData);
    if (includeStaticMap || svg) {
...
function requestMap(ctx) {
    ...
    // are still waiting and dispatch this single request without starting a
    // second runAll timer chain. Position, tracks and statistics are polled
    // independently by the next regular interval.
    ctx.intervalQueue.resetQueue();
    ctx.intervalQueue.add('Generic', 'getMI', { type: '0' });
    ctx.intervalQueue.startNextItemFromQueue(false);
    return true;
}
...
function registerTelemetryBridge(vacbot, ctx) {
    if (!isSupported(ctx) || !vacbot?.dispatcher ||
        typeof vacbot.dispatcher.handleMessagePayload !== 'function') {
        return false;
    }
    const dispatcher = vacbot.dispatcher;
