...
/** Check whether a time uses the 15-minute grid exposed by the official app. */
...
function normalizeDraftTask(task) {
    ...
    if (!isQuarterHour(normalized.sTime) || !isQuarterHour(normalized.eTime)) {
        throw new Error('Task times must use 15-minute steps');
    }
    if (normalized.sDay === normalized.eDay && normalized.sTime === normalized.eTime) {
        throw new Error('Task duration must be at least 15 minutes');
    }
