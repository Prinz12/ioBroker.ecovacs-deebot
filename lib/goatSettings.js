...
/** Convert a GBZ cutting height in centimetres to the O1200 protocol level. */
function heightCmToLevel(heightCm) {
    const numericHeight = Number(heightCm);
    if (!Number.isFinite(numericHeight)) return null;
    const level = 17 - numericHeight * 2;
    return Number.isInteger(level) && level >= 1 && level <= 11 ? level : null;
}

/** Convert the ECOVACS area angle to the value shown by the official app. */
function directionProtocolToApp(angle) {
    const numericAngle = Number(angle);
    if (!Number.isInteger(numericAngle) || numericAngle < 90 || numericAngle > 270) return null;
    return 270 - numericAngle;
}

/** Convert the official app angle to the ECOVACS area protocol value. */
function directionAppToProtocol(angle) {
    const numericAngle = Number(angle);
    if (!Number.isInteger(numericAngle) || numericAngle < 0 || numericAngle > 180) return null;
    return 270 - numericAngle;
}

/** Return the app label and movement speed for a verified GBZ cut mode. */
function cutModeDetails(mode) {
    const numericMode = Number(mode);
    if (numericMode === 7) return { cutModeName: 'fine', cutSpeedMps: 0.35 };
    if (numericMode === 4) return { cutModeName: 'efficient', cutSpeedMps: 0.5 };
    return { cutModeName: 'unknown', cutSpeedMps: null };
}

/** Return the app label and obstacle threshold for a verified GBZ avoidance mode. */
function obstacleModeDetails(level) {
    const numericLevel = Number(level);
    const modes = {
        1: { obstacleModeName: 'shortGrass', obstacleHeightCm: 10 },
        2: { obstacleModeName: 'normal', obstacleHeightCm: 15 },
        3: { obstacleModeName: 'highGrass', obstacleHeightCm: 20 }
    };
    return modes[numericLevel] || { obstacleModeName: 'unknown', obstacleHeightCm: null };
}
...
function handleAreaParameters(ctx, payload) {
    ...
    const areaParameters = payload.areaParameters.map(area => {
        const cutMode = Number(area.cutMode);
        const obstacleHeight = Number(area.obstacleHeight);
        return {
            areaID: String(area.areaID),
            mowHeightLevel: Number(area.mowHeightLevel),
            cutHeightCm: heightLevelToCm(area.mowHeightLevel),
            cutMode,
            ...cutModeDetails(cutMode),
            obstacleHeight,
            ...obstacleModeDetails(obstacleHeight),
            angle: Number(area.angle ?? area.cutDirection),
            appAngle: directionProtocolToApp(area.angle ?? area.cutDirection)
        };
    });
...
function conditionFromState(state) {
    return ({ 0: 'good', 1: 'slightlyWorn', 2: 'heavilyWorn' })[Number(state)] || 'unknown';
}

/** Calculate the percentage and warning state exactly like the official app. */
function getLifeSpanStatus(left, total) {
    if (!Number.isFinite(left) || !Number.isFinite(total) || total <= 0) return null;
    const percent = Math.max(0, Math.ceil(100 * left / total));
    return { percent, state: percent <= 15 ? 2 : percent <= 40 ? 1 : 0 };
}
...
function handleLifeSpan(ctx, payload) {
    ...
    const normalized = entries.filter(entry => LIFE_SPAN_TYPES.includes(entry?.type)).map(entry => {
        ...
        const status = getLifeSpanStatus(left, total);
        return status ? { type: entry.type, left, total, ...status } : null;
    }).filter(Boolean);
...
module.exports = {
    ...
    heightLevelToCm,
    heightCmToLevel,
    directionProtocolToApp,
    directionAppToProtocol,
    cutModeDetails,
    obstacleModeDetails,
    normalizeTime
};
