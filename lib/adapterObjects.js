...
async function createInitialInfoObjects(adapter, ctx) {
    if (ctx && !ctx.skipPrefix && ctx.deviceId) {
        const deviceName = ctx.vacuum.nick || ctx.vacuum.deviceName || ctx.vacuum.name || ctx.deviceId;
        try {
            const obj = await adapter.getObjectAsync(ctx.deviceId);
            if (!obj) {
                await adapter.setObjectNotExistsAsync(ctx.deviceId, {
                    type: 'device',
                    common: {
                        name: deviceName
                    },
            ...
            } else if (obj.type !== 'device' || !obj.common || !obj.common.name || obj.common.name === ctx.deviceId) {
                await adapter.extendObjectAsync(ctx.deviceId, {
                    type: 'device',
                    common: {
                        name: deviceName
                    }
            ...
            adapter.log.warn(`[${ctx.deviceId}] Error creating/updating device object: ${e.message}`);
        }
    }
    ...
    // Removed: the native node-canvas dependency is gone (rendering is pure JS now),
    // so this always-true indicator no longer carries any information.
    await ctx.adapterProxy.deleteObjectIfExists('info.library.canvasModuleIsInstalled');
    ...
    await ctx.adapterProxy.createObjectNotExists(
        ...
        'string', 'text', false, '', '');
    await ctx.adapterProxy.createObjectNotExists(
        'info.platformType', 'Platform type of the device (as classified in the library)',
        'string', 'text', false, '', '');
    await ctx.adapterProxy.createObjectNotExists(
        'info.deviceCategory', 'User-friendly device category (as classified in the library)',
        'string', 'text', false, '', '');
    await ctx.adapterProxy.createObjectNotExists(
        'info.smartType', 'Ecovacs internal identifier (as classified in the library)',
        'string', 'text', false, '', '');
    await ctx.adapterProxy.createObjectNotExists(
...
async function createInitialObjects(adapter, ctx) {
    const isLawnMower = helper.isLawnMowerPlatform(ctx.getPlatformType());
    const supportsLawnMowerControl = helper.supportsLawnMowerControl(
    ...
    if (ctx.getPlatformType() === 'aqMonitor') {
        return;
    }
    ...
    if (ctx.vacbot.hasSpotAreaCleaningMode()) {
        await ctx.adapterProxy.createObjectNotExists(
            ...
            'string', 'value', true, '', '');
        if ((ctx.getPlatformType() === 'legacy') || (ctx.getPlatformType() === '950')) {
            await ctx.adapterProxy.createObjectNotExists(
        ...
        await ctx.adapterProxy.deleteObjectIfExists('control.spotArea_cleanings');
        if (ctx.getPlatformType() !== 'airbot' && !isLawnMower) {
    ...
    if ((ctx.getModel().isSupportedFeature('control.volume')) && (ctx.getPlatformType() !== 'airbot')) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (!isLawnMower) {
        ...
        await ctx.adapterProxy.deleteObjectIfExists('control.clean');
        await ctx.adapterProxy.deleteObjectIfExists('control.clean_home');
        await ctx.adapterProxy.deleteObjectIfExists('control.stop');
        await ctx.adapterProxy.deleteObjectIfExists('control.charge');
        await ctx.adapterProxy.deleteObjectIfExists('control.spot');
        await ctx.adapterProxy.deleteObjectIfExists('control.edge');
    }

    const goatControls = new Map([
        ['startAuto', 'Start automatic mowing'],
        ['startArea', 'Start mowing selected areas'],
        ['startTrim', 'Start trimming selected boundaries'],
        ['pause', 'Pause mowing'],
        ['resume', 'Resume mowing'],
        ['stop', 'Stop mowing'],
        ['goToStation', 'Go to charging station'],
        ['cancelGoToStation', 'Cancel return to charging station']
    ]);
    const goatSettingButtons = new Map([
        ['settingsRefresh', 'Refresh GOAT mowing settings'],
        ['maintenanceRefresh', 'Refresh GOAT maintenance values'],
        ['resetBladeLifeSpan', 'Reset blade maintenance counter'],
        ['resetTrimmerLineLifeSpan', 'Reset trimmer line maintenance counter'],
        ['resetTrimmerBrushLifeSpan', 'Reset trimmer brush maintenance counter'],
        ['loadAreaSettings', 'Load selected area settings'],
        ['applyAreaSettings', 'Apply selected area settings'],
        ['scheduleRefresh', 'Refresh GOAT schedules'],
        ['scheduleLoad', 'Load selected schedule into the draft'],
        ['scheduleCreate', 'Create schedule from the draft'],
        ['scheduleUpdate', 'Update selected schedule from the draft'],
        ['scheduleDelete', 'Delete selected schedule']
    ]);
    const goatSettingValues = new Map([
        ['autoCutDirection', ['Automatic mowing direction', 'boolean', false]],
        ['rainDelayEnabled', ['Rain delay enabled', 'boolean', false]],
        ['rainDelayMinutes', ['Rain delay after rain stops', 'number', 180, 'min']],
        ['animalProtectionEnabled', ['Animal protection enabled', 'boolean', false]],
        ['animalProtectionStart', ['Animal protection start time', 'string', '19:00']],
        ['animalProtectionEnd', ['Animal protection end time', 'string', '07:00']],
        ['aiRecognition', ['AI recognition', 'boolean', false]],
        ['smartTrimmingAvoidance', ['Smart obstacle avoidance while trimming', 'boolean', false]],
        ['narrowPathAdaptation', ['Narrow path adaptation', 'boolean', false]],
        ['settingsAreaId', ['Area ID for mowing settings', 'string', '']],
        ['settingsMowHeightLevel', ['Mowing height level for selected area', 'number', 7]],
        ['settingsMowHeightCm', ['Mowing height for selected area', 'number', 5, 'cm']],
        ['settingsCutMode', ['Mowing efficiency mode for selected area', 'number', 7]],
        ['settingsObstacleHeight', ['Obstacle avoidance height level for selected area', 'number', 2]],
        ['settingsDirection', ['Mowing direction for selected area', 'number', 90, '°']],
        ['settingsDirectionApp', ['Mowing direction as shown in the ECOVACS app', 'number', 180, '°']],
        ['scheduleId', ['Selected schedule ID', 'string', '']],
        ['scheduleName', ['Schedule draft name', 'string', '']],
        ['scheduleEnabled', ['Schedule draft enabled', 'boolean', false]],
        ['scheduleCatchUp', ['Schedule draft catch-up mode', 'boolean', false]],
        ['scheduleTasks', ['Complete schedule draft tasks', 'json', '[]', '', 'json']],
        ['scheduleDeleteConfirm', ['Confirm deletion of selected schedule', 'boolean', false]],
        ['scheduleCatchUpConfirm', ['Confirm enabling catch-up mode', 'boolean', false]],
        ['scheduleActivationConfirm', ['Confirm replacing another active schedule', 'boolean', false]]
    ]);
    if (supportsLawnMowerControl) {
        await ctx.adapterProxy.createChannelNotExists('control.goat', 'GOAT lawn mower controls');
        for (const [objectName, name] of goatControls) {
            await ctx.adapterProxy.createObjectNotExists(
                'control.goat.' + objectName, name,
                'boolean', 'button', true, false, '');
        }
        await ctx.adapterProxy.createObjectNotExists(
            'control.goat.areaIds', 'Area IDs to mow (comma-separated)',
            'string', 'value', true, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'control.goat.trimBoundaryIds', 'Physical boundary IDs to trim (comma-separated)',
            'string', 'value', true, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'control.goat.trimVirtualBoundaryIds', 'Virtual boundary IDs to trim (comma-separated)',
            'string', 'value', true, '', '');
        for (const [objectName, name] of goatSettingButtons) {
            await ctx.adapterProxy.createObjectNotExists(
                'control.goat.' + objectName, name,
                'boolean', 'button', true, false, '');
        }
        for (const [objectName, definition] of goatSettingValues) {
            const [name, type, defaultValue, unit = '', role = 'value'] = definition;
            await ctx.adapterProxy.createObjectNotExists(
                'control.goat.' + objectName, name,
                type, role, true, defaultValue, unit);
        }
        const heightStates = {};
        for (let level = 1; level <= 11; level++) {
            heightStates[level] = `${8.5 - level / 2} cm`;
        }
        const areaSettingMetadata = {
            settingsMowHeightLevel: { min: 1, max: 11, step: 1, states: heightStates },
            settingsMowHeightCm: { min: 3, max: 8, step: 0.5 },
            settingsCutMode: {
                states: { 7: 'Fine (0.35 m/s)', 4: 'Efficient (0.5 m/s)' }
            },
            ...
            settingsDirection: { min: 90, max: 270, step: 1 },
            settingsDirectionApp: { min: 0, max: 180, step: 1 }
        };
        for (const [objectName, common] of Object.entries(areaSettingMetadata)) {
            await ctx.adapterProxy.extendObjectAsync(
                'control.goat.' + objectName, { common }
            );
        }
    } else {
        for (const objectName of goatControls.keys()) {
            await ctx.adapterProxy.deleteObjectIfExists('control.goat.' + objectName);
        }
        await ctx.adapterProxy.deleteObjectIfExists('control.goat.areaIds');
        await ctx.adapterProxy.deleteObjectIfExists('control.goat.trimBoundaryIds');
        await ctx.adapterProxy.deleteObjectIfExists('control.goat.trimVirtualBoundaryIds');
        for (const objectName of goatSettingButtons.keys()) {
            await ctx.adapterProxy.deleteObjectIfExists('control.goat.' + objectName);
        }
        for (const objectName of goatSettingValues.keys()) {
            await ctx.adapterProxy.deleteObjectIfExists('control.goat.' + objectName);
        }
        await ctx.adapterProxy.deleteChannelIfExists('control.goat');
    }
    ...
    if (ctx.getModel().isSupportedFeature('info.dustbox') &&
        ctx.adapter.getHoursUntilDustBagEmptyReminderFlagIsSet(ctx.deviceId) > 0) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    //
    if (!helper.isLawnMowerPlatform(ctx.getPlatformType()) && ctx.getPlatformType() !== 'aqMonitor') {
        await ctx.adapterProxy.createChannelNotExists('consumable', 'Consumable');

        if (ctx.getPlatformType() !== 'airbot') {
            await ctx.adapterProxy.createObjectNotExists(
...
async function createControlWaterLevelIfNotExists(adapter, ctx, def = 2, id = 'control.waterLevel', name = 'Water level') {
    ...
    if (id !== 'control.waterLevel') {
        if (!ctx.getModel().isSupportedFeature('map.spotAreas.waterLevel')) {
            return;
        }
    }
    let min = 1;
...
async function createControlCleanSpeedIfNotExists(adapter, ctx, def = 2, id = 'control.cleanSpeed', name = 'Clean speed') {
    if (id !== 'control.cleanSpeed') {
        if (!ctx.getModel().isSupportedFeature('map.spotAreas.cleanSpeed')) {
            return;
        }
    }
    let states = {
...
async function createAdditionalObjects(adapter, ctx) {
    const platformType = ctx.getPlatformType();
    const supportsLawnMowerControl = helper.supportsLawnMowerControl(
        platformType, ctx.getModel().getDeviceClass()
    );

    // Create network objects for all device types (including aqMonitor)
    if (ctx.getModel().isSupportedFeature('info.network')) {
    ...
    if (platformType === 'aqMonitor') {
        return;
    }

    if (helper.supportsLawnMowerInfo(platformType, ctx.getModel().getDeviceClass())) {
        await ctx.adapterProxy.createChannelNotExists('info.goat', 'GOAT lawn mower information');
        await ctx.adapterProxy.createChannelNotExists('info.goat.settings', 'GOAT mowing settings');
        await ctx.adapterProxy.createChannelNotExists(
            'info.goat.robotSettings', 'GOAT robot settings'
        );
        await ctx.adapterProxy.createChannelNotExists('info.goat.maintenance', 'GOAT maintenance');
        await ctx.adapterProxy.createChannelNotExists('info.goat.schedules', 'GOAT schedules');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.batteryLow', 'Battery is low',
            'boolean', 'indicator.lowbat', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.isCharging', 'Mower is charging',
            'boolean', 'indicator.charging', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.chargeMode', 'Charging mode',
            'string', 'indicator.status', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.workState', 'Mower work state',
            'string', 'indicator.status', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.workTrigger', 'Last work trigger',
            'string', 'value', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.workOther', 'Additional work information',
            'string', 'value', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.cleanType', 'Current mowing type',
            'string', 'value', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.cleanValue', 'Current mowing selection',
            'string', 'value', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.subCleanType', 'Current mowing subtype',
            'string', 'value', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.subCleanValue', 'Current mowing sub-selection',
            'string', 'value', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.motionState', 'Current mower motion state',
            'string', 'indicator.status', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.features', 'GOAT feature flags',
            'json', 'json', false, '{}', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.mapExtension', 'Map extension is supported',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.secondaryCharger', 'Secondary charger is supported',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.videoMoveSupportChannel', 'Video movement channel is supported',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.videoMoveTask', 'Video movement task is supported',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.errorCodes', 'GOAT device error codes',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.hasError', 'GOAT reports an error',
            'boolean', 'indicator.error', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.lastUpdate', 'Timestamp of last GOAT status update',
            'number', 'value.time', false, 0, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.rawResponse', 'Raw GOAT status response',
            'json', 'json', false, '{}', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.areaParameters', 'Per-area mowing settings',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.autoCutDirection', 'Automatic mowing direction',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.rainDelayEnabled', 'Rain delay enabled',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.rainDelayMinutes', 'Rain delay after rain stops',
            'number', 'value.interval', false, 180, 'min');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.animalProtectionEnabled', 'Animal protection enabled',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.animalProtectionStart', 'Animal protection start time',
            'string', 'value.time', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.animalProtectionEnd', 'Animal protection end time',
            'string', 'value.time', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.customCutMode', 'Individual mowing settings enabled',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.borderModeEnabled', 'Border mode enabled',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.borderMode', 'Border mode',
            'number', 'value', false, 0, '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.timezone', 'Mower time zone',
            'string', 'text', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.timezoneOffsetMinutes', 'Mower time zone offset',
            'number', 'value.interval', false, 0, 'min');
        await ctx.adapterProxy.createObjectNotExists(
            'info.goat.settings.lastUpdate', 'Timestamp of last GOAT settings update',
            'number', 'value.time', false, 0, '');
        const robotSettings = [
            ['aiRecognition', 'AI recognition'],
            ['smartTrimmingAvoidance', 'Smart obstacle avoidance while trimming'],
            ['narrowPathAdaptation', 'Narrow path adaptation']
        ];
        for (const [objectName, name] of robotSettings) {
            await ctx.adapterProxy.createObjectNotExists(
                `info.goat.robotSettings.${objectName}`, name,
                'boolean', 'indicator', false, false, '');
        }
        const maintenanceStates = [
            ['rawLifeSpan', 'Raw maintenance values', 'json', 'json', '[]', ''],
            ['bladeRemainingMinutes', 'Blade remaining time', 'number', 'value', 0, 'min'],
            ['bladeTotalMinutes', 'Blade total service life', 'number', 'value', 0, 'min'],
            ['bladeRemainingHours', 'Blade remaining time in rounded-up hours', 'number', 'value', 0, 'h'],
            ['bladePercent', 'Blade remaining percentage', 'number', 'value', 0, '%'],
            ['bladeState', 'Blade maintenance state', 'number', 'value', 0, ''],
            ['bladeCondition', 'Blade condition', 'string', 'text', '', ''],
            ['trimmerLineRemainingUses', 'Trimmer line remaining uses', 'number', 'value', 0, ''],
            ['trimmerLineTotalUses', 'Trimmer line total uses', 'number', 'value', 0, ''],
            ['trimmerLinePercent', 'Trimmer line remaining percentage', 'number', 'value', 0, '%'],
            ['trimmerLineState', 'Trimmer line maintenance state', 'number', 'value', 0, ''],
            ['trimmerLineCondition', 'Trimmer line condition', 'string', 'text', '', ''],
            ['trimmerBrushRemainingDays', 'Trimmer brush remaining days', 'number', 'value', 0, 'd'],
            ['trimmerBrushTotalDays', 'Trimmer brush total service life', 'number', 'value', 0, 'd'],
            ['trimmerBrushPercent', 'Trimmer brush remaining percentage', 'number', 'value', 0, '%'],
            ['trimmerBrushState', 'Trimmer brush maintenance state', 'number', 'value', 0, ''],
            ['trimmerBrushCondition', 'Trimmer brush condition', 'string', 'text', '', ''],
            ['lastUpdate', 'Timestamp of last maintenance update', 'number', 'value.time', 0, '']
        ];
        for (const [objectName, name, type, role, defaultValue, unit] of maintenanceStates) {
            await ctx.adapterProxy.createObjectNotExists(
                `info.goat.maintenance.${objectName}`, name,
                type, role, false, defaultValue, unit);
        }
        const scheduleStates = [
            ['list', 'Decoded GOAT schedules', 'json', 'json', '[]'],
            ['count', 'Number of GOAT schedules', 'number', 'value', 0],
            ['activeScheduleId', 'Active GOAT schedule ID', 'string', 'text', ''],
            ['activeScheduleName', 'Active GOAT schedule name', 'string', 'text', ''],
            ['catchUpEnabled', 'Catch-up mode of the active schedule', 'boolean', 'indicator', false],
            ['lastUpdate', 'Timestamp of last schedule read', 'number', 'value.time', 0],
            ['status', 'Schedule operation status', 'string', 'indicator.status', 'idle'],
            ['lastError', 'Last schedule operation error', 'string', 'text', '']
        ];
        for (const [objectName, name, type, role, defaultValue] of scheduleStates) {
            await ctx.adapterProxy.createObjectNotExists(
                `info.goat.schedules.${objectName}`, name,
                type, role, false, defaultValue, '');
        }
    }
    ...
    // Network objects cleanup for non-network devices (objects created above before aqMonitor guard)
    if (!ctx.getModel().isSupportedFeature('info.network')) {
    ...
    if (supportsLawnMowerControl) {
        ...
        await ctx.adapterProxy.createChannelNotExists('map.goat', 'GOAT lawn map');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.refresh', 'Refresh GOAT map',
            'boolean', 'button', true, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.status', 'GOAT map loading status',
            'string', 'indicator.status', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.lastError', 'Last GOAT map error',
            'string', 'indicator.error', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.mapId', 'GOAT map ID',
            'string', 'text', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.centerX', 'GOAT map center X',
            'number', 'value', false, 0, 'mm');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.centerY', 'GOAT map center Y',
            'number', 'value', false, 0, 'mm');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.squareMeters', 'Mapped lawn area',
            'number', 'value', false, 0, 'm²');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.areaIds', 'Available mowing area IDs',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.areas', 'Mowing area metadata',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.trimBoundaryIds', 'Available physical trim boundary IDs',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.virtualBoundaryIds', 'Available virtual boundary IDs',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.noGoZoneIds', 'Available no-go zone IDs',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.geometry', 'Decoded GOAT map geometry',
            'json', 'json', false, '{}', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.position', 'Current GOAT position',
            'json', 'json', false, '{}', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.positionX', 'Current GOAT X position',
            'number', 'value', false, 0, 'mm');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.positionY', 'Current GOAT Y position',
            'number', 'value', false, 0, 'mm');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.positionValid', 'Current GOAT position is valid',
            'boolean', 'indicator', false, false, '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.positionHeading', 'Current GOAT heading',
            'number', 'value.direction', false, 0, 'deg');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.tracks', 'Current mowing tracks',
            'json', 'json', false, '{}', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.mowedTrail', 'Actually traveled mowing trail',
            'json', 'json', false, '[]', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.completedTrackCount', 'Completed mowing track count',
            'number', 'value', false, 0, '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.remainingTrackCount', 'Remaining mowing track count',
            'number', 'value', false, 0, '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.mowedSquareMeters', 'Mowed area in current run',
            'number', 'value', false, 0, 'm2');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.totalSquareMeters', 'Total area in current run',
            'number', 'value', false, 0, 'm2');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.mowingProgress', 'Mowing progress',
            'number', 'value', false, 0, '%');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.liveLastUpdate', 'Timestamp of last GOAT live-map update',
            'number', 'value.time', false, 0, '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.svg', 'Rendered GOAT map as SVG',
            'string', 'html', false, '', '');
        await ctx.adapterProxy.createObjectNotExists(
            'map.goat.lastUpdate', 'Timestamp of last GOAT map update',
            'number', 'value.time', false, 0, '');
    }
    ...
    if (ctx.getModel().isSupportedFeature('map.deebotPositionCurrentSpotAreaID')) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.vacbot.hasMoppingSystem()) {
        ...
        await ctx.adapterProxy.deleteObjectIfExists('control.waterLevel');
        await ctx.adapterProxy.deleteObjectIfExists('control.waterLevel_standard');
        await ctx.adapterProxy.deleteObjectIfExists('control.waterLevel_reset');
        await ctx.adapterProxy.deleteObjectIfExists('info.waterbox_moppingType');
        await ctx.adapterProxy.deleteObjectIfExists('info.extended.moppingMode');
        await ctx.adapterProxy.deleteObjectIfExists('control.extended.moppingMode');
    ...
    if (ctx.getModel().is950type_V2() && !helper.isLawnMowerPlatform(platformType)) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().is950type_V2() && ctx.vacbot.hasMoppingSystem() && !helper.isLawnMowerPlatform(platformType)) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().is950type() && ctx.vacbot.hasMoppingSystem() && !helper.isLawnMowerPlatform(platformType)) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().isModelTypeT9Based() && !helper.isLawnMowerPlatform(platformType)) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if ((ctx.getModel().isModelTypeT20() || ctx.getModel().isModelTypeX2() || ctx.getModel().isModelTypeX1()) &&
        !helper.isLawnMowerPlatform(platformType)) {
        await ctx.adapterProxy.createObjectNotExists(
    ...
    if (ctx.getModel().hasCleaningStation() && ctx.vacbot.hasHotWaterWashing()) {
