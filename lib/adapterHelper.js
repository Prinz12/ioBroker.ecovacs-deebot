...
/**
 * Legacy: Takes a key and a value, and returns the value decrypted with the key.
 * No longer used in production code; kept for test compatibility.
...
/**
 * Check whether a platform type identifies an Ecovacs lawn mower.
 * The library uses `lawnMower`; older adapter code used `goat`.
 * @param {string} platformType
 * @returns {boolean}
 */
function isLawnMowerPlatform(platformType) {
    return platformType === 'lawnMower' || platformType === 'goat';
}

/**
 * Check whether the read-only Generic status request was verified for this lawn mower model.
 * @param {string} platformType
 * @param {string} deviceClass
 * @returns {boolean}
 */
function supportsLawnMowerInfo(platformType, deviceClass) {
    return isLawnMowerPlatform(platformType) && deviceClass === '2i0fns';
}

/**
 * Check whether GOAT controls were verified from the official app for this model.
 * Keep this separate from read-only polling so future models can be enabled independently.
 * @param {string} platformType
 * @param {string} deviceClass
 * @returns {boolean}
 */
function supportsLawnMowerControl(platformType, deviceClass) {
    return isLawnMowerPlatform(platformType) && deviceClass === '2i0fns';
}
