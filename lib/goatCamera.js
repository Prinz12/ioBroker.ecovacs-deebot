...
class GoatCameraManager {
    ...
    async requestSession(deviceId) {
        ...
        const pending = this.createSession(ctx).finally(() => this.pendingByDevice.delete(ctx.deviceId));
    ...
    async createSession(ctx) {
        ...
        // The native GOAT VideoActivity uses StartUpUtils.initVideo(), which calls the
        // v1 start_watch route. The v2 route is used by a different, PIN-carrying video
        // flow and can return valid AWS credentials without waking this mower's stream.
        const startUrl = `${portalBaseUrl(ctx)}/appsvr/akvs/start_watch`;
