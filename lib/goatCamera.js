...
function hashVideoPin(pin) {
    // Mirrors video_pw_v1: MD5("eco_" + PIN). The clear PIN never leaves the adapter.
    return crypto.createHash('md5').update(`eco_${pin}`).digest('hex');
}
...
class GoatCameraManager {
    ...
    async createSession(ctx, pin) {
        ...
        // Its native video flow passes the verified MD5 video PIN to start_watch/v2.
