...
const GOAT_VIDEO_RESOLUTIONS = new Set([0, 1]);
...
function ecovacsHeaders(ctx) {
    ...
    const accountUid = String(ctx.api?.accountUid || '');
    return {
...
class GoatCameraManager {
    ...
    async requestSessionWithResolution(deviceId, resolution) {
        if (!GOAT_VIDEO_RESOLUTIONS.has(resolution)) {
            throw new Error('GOAT video resolution must be 0 (Normal) or 1 (Clear)');
        }
        const session = await this.requestSession(deviceId);
        try {
            const selected = await this.setResolution(session.sessionId, resolution);
    ...
    async setResolution(id, resolution) {
        if (!GOAT_VIDEO_RESOLUTIONS.has(resolution)) {
            throw new Error('GOAT video resolution must be 0 (Normal) or 1 (Clear)');
        }
        const session = this.sessions.get(String(id || ''));
        if (!session) {
            throw new Error('GOAT camera session is not active');
        }
        ...
        let deviceCode;
        if (originalResolver) {
            deviceSession._resolveImmediatePayload = function captureGoatResolutionCode(command, responseData) {
                if (String(command?.name || '').toLowerCase() === 'setvideoresolution') {
                    const code = Number(responseData?.resp?.body?.code);
                    if (Number.isInteger(code)) deviceCode = code;
                }
                return originalResolver.apply(this, arguments);
        ...
        try {
            await ctx.vacbot.runAsync('Generic', 'setVideoResolution', { resolution }, { timeoutMs: 15000 });
        } catch (error) {
            throw externalRequestError('GOAT video resolution', error);
        } finally {
            if (originalResolver) deviceSession._resolveImmediatePayload = originalResolver;
        }
        if (Number.isInteger(deviceCode) && deviceCode !== 0) {
            throw new Error(`GOAT rejected the video resolution (device code ${deviceCode})`);
        }
        session.resolution = resolution;
        const label = resolution === 1 ? 'Clear' : 'Normal';
        this.adapter.log.info(`GOAT camera resolution set to ${label}`);
        return { resolution, resolutionLabel: label };
    }
