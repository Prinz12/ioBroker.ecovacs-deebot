...
const goatSettings = require('./goatSettings');
const goatSchedules = require('./goatSchedules');
const mapHelper = require('./mapHelper');
const C = require('./constants');
const commandRegistry = require('./commandRegistry').createRegistry();

/**
 * Clone a generic command payload before the library base class adds its request id.
 * @param {object|Array|undefined} payload
 * @returns {object|Array}
 */
function cloneGenericCommandPayload(payload) {
...
/**
 * Restore the Generic command class expected by the command dispatcher.
 * @param {object} ctx
 */
function ensureGenericCommandCompatibility(ctx) {
    const commandClass = ctx.vacbot.vacBotCommand;
    // TODO: Remove this compatibility class once ecovacs-deebot exports Generic again.
    if ((typeof commandClass === 'function') && (typeof commandClass.Generic !== 'function')) {
        commandClass.Generic = class Generic extends commandClass {
            /**
             * Create a generic command without mutating the caller's payload.
             * @param {string} name
             * @param {object|Array} [payload]
             */
            constructor(name, payload) {
                super(name, cloneGenericCommandPayload(payload));
            }
        };
    }
}

const GOAT_MOWING_TYPES = new Set([
    'auto',
    'spotArea',
    'border',
    'remoteMow',
...
/**
 * Send a command whose envelope and payload were verified from the official GOAT app.
 * @param {object} ctx
 * @param {string} command
 * @param {object} payload
 * @returns {boolean} whether the command was dispatched
 */
function runVerifiedGoatCommand(ctx, command, payload) {
    if (!helper.supportsLawnMowerControl(
        ctx.getPlatformType(), ctx.getModel().getDeviceClass()
    )) {
        ctx.adapter.log.warn('GOAT control is not verified for this mower model');
        return false;
    }
    ensureGenericCommandCompatibility(ctx);
    ctx.adapter.log.info(`Run GOAT command: ${command} ${JSON.stringify(payload)}`);
    ctx.vacbot.run('Generic', command, payload);
    return true;
}

/**
 * Read the current mowing type reported by getCleanInfo. Pause/resume/stop must not guess it.
 * @param {object} ctx
 * @returns {Promise<string|null>}
 */
async function getCurrentGoatMowingType(ctx) {
    const state = await ctx.adapterProxy.getStateAsync('info.goat.cleanType');
    const cleanType = typeof state?.val === 'string' ? state.val : '';
    if (!GOAT_MOWING_TYPES.has(cleanType)) {
        ctx.adapter.log.warn('Skip GOAT command: no supported active mowing type is available');
        return null;
    }
    return cleanType;
}
...
/**
 * Parse a comma-separated list of numeric GOAT map object IDs.
 * @param {unknown} value
 * @returns {Array<string>|null}
 */
function parseGoatIdSelection(value) {
...
/**
 * Read an array-valued JSON state containing available GOAT map IDs.
 * @param {object} ctx
 * @param {string} stateId
 * @returns {Promise<Array<string>>}
 */
async function getAvailableGoatIds(ctx, stateId) {
    const state = await ctx.adapterProxy.getStateAsync(stateId);
    try {
        const values = JSON.parse(typeof state?.val === 'string' ? state.val : '[]');
...
/**
 * Validate selected IDs against the most recently read map.
 * @param {object} ctx
 * @param {string} selectionStateId
 * @param {string} availableStateId
 * @param {string} label
 * @returns {Promise<Array<string>|null>}
 */
async function getValidatedGoatSelection(ctx, selectionStateId, availableStateId, label) {
    const selectionState = await ctx.adapterProxy.getStateAsync(selectionStateId);
    const selected = parseGoatIdSelection(selectionState?.val);
    if (selected === null) {
        ctx.adapter.log.warn(`Skip GOAT command: invalid ${label} selection`);
        return null;
    }
    const available = await getAvailableGoatIds(ctx, availableStateId);
    const unknown = selected.filter(id => !available.includes(id));
    if (unknown.length) {
        ctx.adapter.log.warn(`Skip GOAT command: unknown ${label} IDs ${unknown.join(',')}`);
        return null;
    }
    return selected;
}
...
/**
 * Verify that the mower is idle before starting a new selection.
 * @param {object} ctx
 * @returns {Promise<boolean>}
 */
async function isGoatIdle(ctx, action = 'start command') {
    const state = await ctx.adapterProxy.getStateAsync('info.goat.workState');
    if (state?.val !== 'idle') {
        ctx.adapter.log.warn(`Skip GOAT ${action}: mower is not idle (${state?.val || 'unknown'})`);
        return false;
    }
    return true;
}

/** Queue the two read-only requests used by the official mowing settings page. */
function requestGoatSettings(ctx) {
    if (!helper.supportsLawnMowerInfo(
        ctx.getPlatformType(), ctx.getModel().getDeviceClass()
    )) {
        ctx.adapter.log.warn('GOAT settings are not verified for this mower model');
        return false;
    }
    ctx.commandQueue.addLawnMowerSettings();
    ctx.commandQueue.runAll();
    return true;
}

/** Queue only the read-only GOAT maintenance request. */
function requestGoatMaintenance(ctx) {
    if (!helper.supportsLawnMowerInfo(
        ctx.getPlatformType(), ctx.getModel().getDeviceClass()
    )) {
        ctx.adapter.log.warn('GOAT maintenance is not verified for this mower model');
        return false;
    }
    ctx.commandQueue.addLawnMowerMaintenance();
    ctx.commandQueue.runAll();
    return true;
}

/** Schedule a settings read-back after a verified write command. */
function scheduleGoatSettingsReadback(ctx) {
    clearTimeout(ctx._goatSettingsReadbackTimeout);
    ctx._goatSettingsReadbackTimeout = setTimeout(() => {
        requestGoatSettings(ctx);
    }, 2000);
}

/** Schedule a maintenance read-back after a verified reset command. */
function scheduleGoatMaintenanceReadback(ctx) {
    clearTimeout(ctx._goatMaintenanceReadbackTimeout);
    ctx._goatMaintenanceReadbackTimeout = setTimeout(() => {
        requestGoatMaintenance(ctx);
    }, 2000);
}

/** Schedule a read-back after one GOAT schedule write. */
function scheduleGoatSchedulesReadback(ctx) {
    clearTimeout(ctx._goatSchedulesReadbackTimeout);
    ctx._goatSchedulesReadbackTimeout = setTimeout(() => {
        goatSchedules.requestSchedules(ctx);
    }, 2000);
}

/** Read one boolean confirmation state used for destructive schedule operations. */
async function hasScheduleConfirmation(ctx, stateName) {
    const state = await ctx.adapterProxy.getStateAsync(`control.goat.${stateName}`);
    return state?.val === true;
}

/** Clear the one-shot schedule confirmation states after a write attempt. */
function clearScheduleConfirmations(ctx) {
    for (const stateName of [
        'scheduleDeleteConfirm', 'scheduleCatchUpConfirm', 'scheduleActivationConfirm'
    ]) {
        ctx.adapterProxy.setStateConditional(`control.goat.${stateName}`, false, true);
    }
}

/** Validate, send and read back one official setSchedules operation. */
async function applyGoatScheduleAction(ctx, action) {
    try {
        if (!(await isGoatIdle(ctx, 'schedule change'))) {
            throw new Error('The mower must be idle before changing schedules');
        }
        let draft;
        let current;
        if (action === 'del') {
            const idState = await ctx.adapterProxy.getStateAsync('control.goat.scheduleId');
            const scheduleId = String(idState?.val ?? '').trim();
            current = goatSchedules.getCachedSchedule(ctx, scheduleId);
            if (!(await hasScheduleConfirmation(ctx, 'scheduleDeleteConfirm'))) {
                throw new Error('Set scheduleDeleteConfirm before deleting a schedule');
            }
            draft = { sid: scheduleId };
        } else {
            draft = await goatSchedules.readDraft(ctx);
            current = action === 'mod' ? goatSchedules.getCachedSchedule(ctx, draft.sid) : null;
            const duplicateName = (ctx.goatSchedules || []).find(schedule =>
                schedule.name === draft.name && schedule.sid !== current?.sid
            );
            if (duplicateName) throw new Error(`A schedule named ${draft.name} already exists`);
            if (draft.rotation && current?.rotation !== 1
                && !(await hasScheduleConfirmation(ctx, 'scheduleCatchUpConfirm'))) {
                throw new Error('Set scheduleCatchUpConfirm before enabling catch-up mode');
            }
            const active = (ctx.goatSchedules || []).find(schedule => schedule.using === 1);
            if (draft.using && active && active.sid !== current?.sid
                && !(await hasScheduleConfirmation(ctx, 'scheduleActivationConfirm'))) {
                throw new Error('Set scheduleActivationConfirm before replacing the active schedule');
            }
        }
        const payload = goatSchedules.buildPayload(action, draft, current);
        if (!runVerifiedGoatCommand(ctx, 'setSchedules', payload)) {
            throw new Error('Schedule commands are not verified for this mower model');
        }
        goatSchedules.setStatus(ctx, 'pending');
        scheduleGoatSchedulesReadback(ctx);
    } catch (error) {
        ctx.adapter.log.warn(`Skip GOAT schedule operation: ${error.message}`);
        goatSchedules.setStatus(ctx, 'error', error.message);
    } finally {
        clearScheduleConfirmations(ctx);
    }
}

/** Send one exact boolean robot setting payload used by the official app. */
async function setGoatBooleanSetting(ctx, action, command, payloadKey, value) {
    if (typeof value !== 'boolean' || !(await isGoatIdle(ctx, action))) return;
    if (runVerifiedGoatCommand(ctx, command, { [payloadKey]: value ? 1 : 0 })) {
        scheduleGoatSettingsReadback(ctx);
    }
}

/** Reset one verified GOAT maintenance counter while the mower is idle. */
async function resetGoatLifeSpan(ctx, type) {
    if (!(await isGoatIdle(ctx, 'maintenance reset'))) return;
    if (runVerifiedGoatCommand(ctx, 'resetLifeSpan', { type })) {
        scheduleGoatMaintenanceReadback(ctx);
    }
}

/** Parse and validate one time value for the GOAT animal protection schedule. */
function normalizeGoatTime(value) {
...
/** Read one ioBroker state value used to compose a complete setting payload. */
async function getGoatStateValue(ctx, stateId) {
    const state = await ctx.adapterProxy.getStateAsync(stateId);
    return state?.val;
}

/** Send the complete rain-delay payload used by the official app. */
async function setGoatRainDelay(ctx, enabledOverride, delayOverride) {
    if (!(await isGoatIdle(ctx, 'rain setting'))) return;
    const enabledValue = enabledOverride ?? await getGoatStateValue(
        ctx, 'info.goat.settings.rainDelayEnabled'
    );
    const delayValue = delayOverride ?? await getGoatStateValue(
        ctx, 'info.goat.settings.rainDelayMinutes'
    );
    const delay = Number(delayValue);
    if (!Number.isInteger(delay) || delay < 0 || delay > 720 || delay % 30 !== 0) {
        ctx.adapter.log.warn('Skip GOAT rain setting: delay must be 0-720 minutes in 30-minute steps');
        return;
    }
    if (runVerifiedGoatCommand(ctx, 'setRainDelay', {
        enable: enabledValue ? 1 : 0,
        delay
    })) {
        scheduleGoatSettingsReadback(ctx);
    }
...
/** Send the complete animal-protection payload used by the official app. */
async function setGoatAnimalProtection(ctx, overrides = {}) {
    if (!(await isGoatIdle(ctx, 'animal protection setting'))) return;
    const enabledValue = overrides.enable ?? await getGoatStateValue(
        ctx, 'info.goat.settings.animalProtectionEnabled'
    );
    const startValue = overrides.start ?? await getGoatStateValue(
        ctx, 'info.goat.settings.animalProtectionStart'
    );
    const endValue = overrides.end ?? await getGoatStateValue(
        ctx, 'info.goat.settings.animalProtectionEnd'
    );
    const start = normalizeGoatTime(startValue);
    const end = normalizeGoatTime(endValue);
    if (!start || !end) {
        ctx.adapter.log.warn('Skip GOAT animal protection setting: invalid start or end time');
        return;
    }
    if (runVerifiedGoatCommand(ctx, 'setAnimProtect', {
        enable: enabledValue ? 1 : 0,
        start,
        end
    })) {
...
/** Read the latest per-area settings array from ioBroker. */
async function getGoatAreaParameters(ctx) {
    const value = await getGoatStateValue(ctx, 'info.goat.settings.areaParameters');
    try {
...
/** Load one selected area's current values into the local staging states. */
async function loadGoatAreaSettings(ctx) {
    const areaId = String(await getGoatStateValue(ctx, 'control.goat.settingsAreaId') ?? '');
    const area = (await getGoatAreaParameters(ctx)).find(item => String(item.areaID) === areaId);
    if (!area) {
        ctx.adapter.log.warn(`Cannot load GOAT area settings: unknown area ID ${areaId || '(empty)'}`);
        return false;
    }
    const stagedValues = {
        settingsMowHeightLevel: area.mowHeightLevel,
        settingsMowHeightCm: area.cutHeightCm ?? goatSettings.heightLevelToCm(area.mowHeightLevel),
        settingsCutMode: area.cutMode,
        settingsObstacleHeight: area.obstacleHeight,
        settingsDirection: area.angle,
        settingsDirectionApp: area.appAngle ?? goatSettings.directionProtocolToApp(area.angle)
    };
    for (const [stateName, value] of Object.entries(stagedValues)) {
        ctx.adapterProxy.setStateConditional(`control.goat.${stateName}`, Number(value), true);
    }
    return true;
}

/** Validate and send one complete per-area mowing settings payload. */
async function applyGoatAreaSettings(ctx) {
    if (!(await isGoatIdle(ctx, 'area setting'))) return;
    const areaId = String(await getGoatStateValue(ctx, 'control.goat.settingsAreaId') ?? '');
    const currentArea = (await getGoatAreaParameters(ctx))
        .find(item => String(item.areaID) === areaId);
    if (!currentArea) {
        ctx.adapter.log.warn(`Skip GOAT area setting: unknown area ID ${areaId || '(empty)'}`);
        return;
    }
    const heightCm = Number(await getGoatStateValue(ctx, 'control.goat.settingsMowHeightCm'));
    const directionApp = Number(await getGoatStateValue(ctx, 'control.goat.settingsDirectionApp'));
    const payload = {
        areaID: areaId,
        mowHeightLevel: goatSettings.heightCmToLevel(heightCm),
        cutMode: Number(await getGoatStateValue(ctx, 'control.goat.settingsCutMode')),
        obstacleHeight: Number(await getGoatStateValue(ctx, 'control.goat.settingsObstacleHeight')),
        angle: goatSettings.directionAppToProtocol(directionApp)
    };
    const valid = Number.isInteger(payload.mowHeightLevel) && payload.mowHeightLevel >= 1
        && payload.mowHeightLevel <= 11
        && Number.isInteger(payload.cutMode) && [4, 7].includes(payload.cutMode)
        && Number.isInteger(payload.obstacleHeight) && payload.obstacleHeight >= 1
        && payload.obstacleHeight <= 3
        && Number.isInteger(payload.angle) && payload.angle >= 90 && payload.angle <= 270;
    if (!valid) {
        ctx.adapter.log.warn('Skip GOAT area setting: invalid height, mode, obstacle height or direction');
        return;
    }
    if (runVerifiedGoatCommand(ctx, 'setAreaParameter', payload)) {
        scheduleGoatSettingsReadback(ctx);
    }
}
...
function registerAllCommands() {
    ...
    commandRegistry.register('control.goat.startAuto', (adapter, ctx, state) => {
        if (state.val !== true) return;
        runVerifiedGoatCommand(ctx, 'clean', { act: 'start', content: { type: 'auto' } });
    });
    commandRegistry.register('control.goat.startArea', async (adapter, ctx, state) => {
        if (state.val !== true || !(await isGoatIdle(ctx))) return;
        const areaIds = await getValidatedGoatSelection(
            ctx, 'control.goat.areaIds', 'map.goat.areaIds', 'area'
        );
        if (!areaIds?.length) {
            ctx.adapter.log.warn('Skip GOAT area mowing: no area IDs selected');
            return;
        }
        runVerifiedGoatCommand(ctx, 'clean', {
            act: 'start',
            content: { type: 'spotArea', value: areaIds.join(',') }
        });
    });
    commandRegistry.register('control.goat.startTrim', async (adapter, ctx, state) => {
        if (state.val !== true || !(await isGoatIdle(ctx))) return;
        const boundaryIds = await getValidatedGoatSelection(
            ctx, 'control.goat.trimBoundaryIds', 'map.goat.trimBoundaryIds', 'trim boundary'
        );
        const virtualIds = await getValidatedGoatSelection(
            ctx, 'control.goat.trimVirtualBoundaryIds', 'map.goat.virtualBoundaryIds', 'virtual boundary'
        );
        if (boundaryIds === null || virtualIds === null) return;
        const selection = [
            ...boundaryIds.map(id => `reid:${id}`),
            ...virtualIds.map(id => `vid:${id}`)
        ];
        if (!selection.length) {
            ctx.adapter.log.warn('Skip GOAT trimming: no boundary IDs selected');
            return;
        }
        runVerifiedGoatCommand(ctx, 'clean', {
            act: 'start',
            content: { type: 'borderrotate', value: selection.join(';') }
        });
    });
    for (const action of ['pause', 'resume', 'stop']) {
        commandRegistry.register(`control.goat.${action}`, async (adapter, ctx, state) => {
            if (state.val !== true) return;
            const cleanType = await getCurrentGoatMowingType(ctx);
            if (cleanType) {
                runVerifiedGoatCommand(ctx, 'clean', { act: action, content: { type: cleanType } });
            }
        });
    }
    commandRegistry.register('control.goat.goToStation', (adapter, ctx, state) => {
        if (state.val !== true) return;
        runVerifiedGoatCommand(ctx, 'charge', { act: 'go' });
    });
    commandRegistry.register('control.goat.cancelGoToStation', (adapter, ctx, state) => {
        if (state.val !== true) return;
        runVerifiedGoatCommand(ctx, 'charge', { act: 'stop' });
    });
    commandRegistry.register('control.goat.settingsRefresh', (adapter, ctx, state) => {
        if (state.val !== true) return;
        requestGoatSettings(ctx);
    });
    commandRegistry.register('control.goat.maintenanceRefresh', (adapter, ctx, state) => {
        if (state.val !== true) return;
        requestGoatMaintenance(ctx);
    });
    commandRegistry.register('control.goat.scheduleRefresh', (adapter, ctx, state) => {
        if (state.val !== true) return;
        goatSchedules.requestSchedules(ctx);
    });
    commandRegistry.register('control.goat.scheduleLoad', async (adapter, ctx, state) => {
        if (state.val !== true) return;
        try {
            const idState = await ctx.adapterProxy.getStateAsync('control.goat.scheduleId');
            goatSchedules.loadDraft(ctx, String(idState?.val ?? '').trim());
            goatSchedules.setStatus(ctx, 'ready');
        } catch (error) {
            ctx.adapter.log.warn(`Cannot load GOAT schedule: ${error.message}`);
            goatSchedules.setStatus(ctx, 'error', error.message);
        }
    });
    commandRegistry.register('control.goat.scheduleCreate', (adapter, ctx, state) => {
        if (state.val === true) return applyGoatScheduleAction(ctx, 'add');
    });
    commandRegistry.register('control.goat.scheduleUpdate', (adapter, ctx, state) => {
        if (state.val === true) return applyGoatScheduleAction(ctx, 'mod');
    });
    commandRegistry.register('control.goat.scheduleDelete', (adapter, ctx, state) => {
        if (state.val === true) return applyGoatScheduleAction(ctx, 'del');
    });
    for (const stateName of [
        'scheduleId', 'scheduleName', 'scheduleEnabled', 'scheduleCatchUp', 'scheduleTasks',
        'scheduleDeleteConfirm', 'scheduleCatchUpConfirm', 'scheduleActivationConfirm'
    ]) {
        commandRegistry.register(`control.goat.${stateName}`, (adapter, ctx, state) => {
            ctx.adapterProxy.setStateConditional(`control.goat.${stateName}`, state.val, true);
        });
    }
    commandRegistry.register('control.goat.autoCutDirection', async (adapter, ctx, state) => {
        if (typeof state.val !== 'boolean' || !(await isGoatIdle(ctx, 'direction setting'))) return;
        if (runVerifiedGoatCommand(ctx, 'setAutoCutDirection', { enable: state.val ? 1 : 0 })) {
            scheduleGoatSettingsReadback(ctx);
        }
    });
    commandRegistry.register('control.goat.rainDelayEnabled', async (adapter, ctx, state) => {
        if (typeof state.val !== 'boolean') return;
        await setGoatRainDelay(ctx, state.val, undefined);
    });
    commandRegistry.register('control.goat.rainDelayMinutes', async (adapter, ctx, state) => {
        await setGoatRainDelay(ctx, undefined, state.val);
    });
    commandRegistry.register('control.goat.animalProtectionEnabled', async (adapter, ctx, state) => {
        if (typeof state.val !== 'boolean') return;
        await setGoatAnimalProtection(ctx, { enable: state.val });
    });
    commandRegistry.register('control.goat.animalProtectionStart', async (adapter, ctx, state) => {
        await setGoatAnimalProtection(ctx, { start: state.val });
    });
    commandRegistry.register('control.goat.animalProtectionEnd', async (adapter, ctx, state) => {
        await setGoatAnimalProtection(ctx, { end: state.val });
    });
    commandRegistry.register('control.goat.aiRecognition', async (adapter, ctx, state) => {
        await setGoatBooleanSetting(
            ctx, 'AI recognition setting', 'setRecognization', 'state', state.val
        );
    });
    commandRegistry.register('control.goat.smartTrimmingAvoidance', async (adapter, ctx, state) => {
        await setGoatBooleanSetting(
            ctx, 'trimming avoidance setting', 'setHumanoidAI', 'enable', state.val
        );
    });
    commandRegistry.register('control.goat.narrowPathAdaptation', async (adapter, ctx, state) => {
        await setGoatBooleanSetting(
            ctx, 'narrow path setting', 'setNarrowAdapt', 'state', state.val
        );
    });
    const maintenanceResets = {
        resetBladeLifeSpan: 'blade',
        resetTrimmerLineLifeSpan: 'weedRope',
        resetTrimmerBrushLifeSpan: 'trimmerBrush'
    };
    for (const [stateName, type] of Object.entries(maintenanceResets)) {
        commandRegistry.register(`control.goat.${stateName}`, async (adapter, ctx, state) => {
            if (state.val !== true) return;
            await resetGoatLifeSpan(ctx, type);
        });
    }
    commandRegistry.register('control.goat.settingsAreaId', async (adapter, ctx, state) => {
        const areaId = String(state.val ?? '').trim();
        if (!/^\d+$/.test(areaId)) return;
        ctx.adapterProxy.setStateConditional('control.goat.settingsAreaId', areaId, true);
    });
    commandRegistry.register('control.goat.settingsMowHeightLevel', (adapter, ctx, state) => {
        const level = Number(state.val);
        const heightCm = goatSettings.heightLevelToCm(level);
        if (!Number.isInteger(level) || level < 1 || level > 11 || heightCm === null) return;
        ctx.adapterProxy.setStateConditional('control.goat.settingsMowHeightLevel', level, true);
        ctx.adapterProxy.setStateConditional('control.goat.settingsMowHeightCm', heightCm, true);
    });
    commandRegistry.register('control.goat.settingsMowHeightCm', (adapter, ctx, state) => {
        const heightCm = Number(state.val);
        const level = goatSettings.heightCmToLevel(heightCm);
        if (level === null) return;
        ctx.adapterProxy.setStateConditional('control.goat.settingsMowHeightCm', heightCm, true);
        ctx.adapterProxy.setStateConditional('control.goat.settingsMowHeightLevel', level, true);
    });
    commandRegistry.register('control.goat.settingsDirection', (adapter, ctx, state) => {
        const protocolAngle = Number(state.val);
        const appAngle = goatSettings.directionProtocolToApp(protocolAngle);
        if (appAngle === null) return;
        ctx.adapterProxy.setStateConditional('control.goat.settingsDirection', protocolAngle, true);
        ctx.adapterProxy.setStateConditional('control.goat.settingsDirectionApp', appAngle, true);
    });
    commandRegistry.register('control.goat.settingsDirectionApp', (adapter, ctx, state) => {
        const appAngle = Number(state.val);
        const protocolAngle = goatSettings.directionAppToProtocol(appAngle);
        if (protocolAngle === null) return;
        ctx.adapterProxy.setStateConditional('control.goat.settingsDirectionApp', appAngle, true);
        ctx.adapterProxy.setStateConditional('control.goat.settingsDirection', protocolAngle, true);
    });
    for (const stateName of [
        'settingsCutMode',
        'settingsObstacleHeight'
    ]) {
        commandRegistry.register(`control.goat.${stateName}`, (adapter, ctx, state) => {
            if (!Number.isFinite(Number(state.val))) return;
            ctx.adapterProxy.setStateConditional(`control.goat.${stateName}`, Number(state.val), true);
        });
    }
    commandRegistry.register('control.goat.loadAreaSettings', async (adapter, ctx, state) => {
        if (state.val !== true) return;
        await loadGoatAreaSettings(ctx);
    });
    commandRegistry.register('control.goat.applyAreaSettings', async (adapter, ctx, state) => {
        if (state.val !== true) return;
        await applyGoatAreaSettings(ctx);
    });
    ...
    commandRegistry.register('map.goat.refresh', (adapter, ctx, state) => {
        if (state.val !== true) return;
        adapter.setState(ctx.statePath('map.goat.refresh'), false, true);
        goatMap.requestMap(ctx);
    });
...
async function handleStateChange(adapter, ctx, id, state) {
    ...
    const handler = (subChannelName && commandRegistry.get(`${channelName}.${subChannelName}.${stateName}`)) ||
        commandRegistry.get(`${channelName}.${stateName}`);
    if (handler) {
...
module.exports = {
    cleanSpotArea,
    ensureGenericCommandCompatibility,
    handleStateChange,
    runSetCleanSpeed,
    handleV2commands,
