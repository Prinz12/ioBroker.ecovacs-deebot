...
async function applyGoatAreaSettings(ctx) {
    ...
    const payload = {
        areaID: areaId,
        mowHeightLevel: Number(await getGoatStateValue(ctx, 'control.goat.settingsMowHeightLevel')),
    ...
    const valid = Number.isInteger(payload.mowHeightLevel) && payload.mowHeightLevel >= 1
        ...
        && Number.isInteger(payload.cutMode) && payload.cutMode >= 4 && payload.cutMode <= 7
        && Number.isInteger(payload.obstacleHeight) && payload.obstacleHeight >= 1
        && payload.obstacleHeight <= 3
        && Number.isInteger(payload.angle) && payload.angle >= 30 && payload.angle <= 360
        && payload.angle % 30 === 0;
    if (!valid) {
...
function registerAllCommands() {
    ...
    for (const stateName of [
        'settingsMowHeightLevel', 'settingsCutMode',
