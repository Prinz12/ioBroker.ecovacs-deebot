...
The experimental camera viewer uses the same short-lived AWS Kinesis Video Streams WebRTC session as the Android app. Configure the four-digit **GOAT Video Manager PIN** in the adapter settings, then open `/ecovacs-deebot/goat-camera.html?instance=0&deviceId=<device-did>` through the ioBroker web adapter. The stream starts only after pressing **Kamera öffnen**, stops explicitly when the page is closed, and is force-closed by the adapter after five minutes. The PIN, ECOVACS account token, and raw AWS access-key pair remain in the adapter process; the browser receives only the signed WebRTC session data required for that short connection. Because the live picture is privacy-sensitive, use an authenticated ioBroker web instance and do not expose the viewer publicly.
