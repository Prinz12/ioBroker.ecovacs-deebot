...
- `scheduleRefresh` plus `info.goat.schedules.*` for decoded GOAT schedules; `scheduleLoad`, `scheduleCreate`, `scheduleUpdate` and `scheduleDelete` provide staged CRUD operations
...
Schedule editing is also staged: select `scheduleId` and press `scheduleLoad`, edit `scheduleName`, `scheduleEnabled`, `scheduleCatchUp`, and the complete `scheduleTasks` JSON array, then deliberately press `scheduleUpdate`. New tasks omit `ssid`; existing tasks keep the `ssid` returned in `info.goat.schedules.list`. Start and end times must use the official app's 15-minute grid and a same-day task must be at least 15 minutes long. The adapter derives the required `taskAct` values and reads the list back after each write. Deletion requires `scheduleDeleteConfirm=true`; enabling catch-up requires `scheduleCatchUpConfirm=true`; replacing another active schedule requires `scheduleActivationConfirm=true`. These confirmations are one-shot and reset after the attempt. Catch-up is rejected for trimming tasks, matching the official app.

The experimental camera viewer uses the same short-lived AWS Kinesis Video Streams WebRTC session as the Android app. Configure the four-digit **GOAT Video Manager PIN** in the adapter settings, then open `/ecovacs-deebot/goat-camera.html?instance=0&deviceId=<device-did>` through the ioBroker web adapter. The stream starts only after pressing **Kamera öffnen**, stops explicitly when the page is closed, and is force-closed by the adapter after five minutes. The resolution selector uses the two values verified from the Android app: **Normal** (`0`) and **Clear/Klar** (`1`); it can be chosen before starting or changed during an active session. The PIN, ECOVACS account token, and raw AWS access-key pair remain in the adapter process; the browser receives only the signed WebRTC session data required for that short connection. Because the live picture is privacy-sensitive, use an authenticated ioBroker web instance and do not expose the viewer publicly.
